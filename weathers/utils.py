...
def getApiData2(apiType, lat_code, lon_code):
    apiURL = 'http://api.openweathermap.org/data/3.0/{}?lat={}&lon={}&appid={}&units=metric'.format(apiType, lat_code, lon_code, APIKEY)
    apiData = requests.get(apiURL).json()
    return apiData
...
def getOneDayData(lat_code, lon_code):
    OneDayData = getApiData2(ONEDAY, lat_code, lon_code)
